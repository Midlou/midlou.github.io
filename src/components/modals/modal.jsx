...
const Modal = ({ isOpen = false, setIsOpen, title = '', children = null, footer = null }) => {
	if (!isOpen) return null;
	return <Transition appear show={isOpen} as={Fragment}>
		<Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto text-white" onClose={() => setIsOpen(false)}>
			<div className="min-h-screen px-4 text-center">
				...
				<Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95"
					...
					<div className="inline-block w-full max-w-lg p-10 overflow-hidden text-left align-middle transition-all transform bg-gray-800 shadow-lg rounded-sm">
						<Dialog.Title as="h1" className="text-4xl font-medium">
							{title}
						</Dialog.Title>
